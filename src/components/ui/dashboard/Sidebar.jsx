...
//       <aside
//         className={`fixed inset-y-0 right-0 z-50 w-80 bg-white/95 dark:bg-[#1c283b]/95 backdrop-blur-md border-l border-gray-200/50 dark:border-gray-700/50 shadow-sm transition-all duration-300 md:static md:translate-x-0 ${
...
//             <div className="w-10 h-10 bg-gradient-to-br from-[#1c283b] to-[#2d3748] rounded-xl flex items-center justify-center shadow-sm">
...
//                       isActive
//                         ? "bg-gradient-to-r from-[#1c283b] to-[#2d3748] text-white shadow-sm transform scale-105"
...
export default function Sidebar({ isOpen, onClose }) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 right-0 z-50 w-80 bg-[#fff] dark:bg-[#162034] backdrop-blur-md shadow-md transition-all duration-300 md:static md:translate-x-0 ${
          isOpen ? "translate-x-0" : "translate-x-full"
        } md:flex flex-col`}
      >
        {/* Header */}
        <div className="h-16 flex items-center justify-between border-b border-[#cccccc5e] px-6 bg-muted/30">
          <div className="flex items-center gap-3">
          ...
          <button
            onClick={onClose}
            className="md:hidden p-2.5 rounded-xl hover:bg-muted transition-all duration-200 hover:scale-105"
          >
                  ...
                  <Link
                    ...
                    className={`flex items-center gap-4 px-4 py-3.5 rounded-xl text-sm font-semibold transition-all duration-200 group ${
                      isActive
                        ...
                        : /* Updated inactive menu item colors to use semantic tokens */ "text-sidebar-foreground hover:text-sidebar-accent hover:bg-muted/50 hover:scale-105"
                    }`}
                  >
                    <Icon
                      className={`h-5 w-5 transition-all duration-200 ${
                    ...
                    {item.name}
                    {isActive && (
                      <div className="ml-auto w-2 h-2 bg-sidebar-primary-foreground rounded-full shadow-sm"></div>
                    )}
                  </Link>
                );
              })}
        ...
        {/* Footer */}
        <div className="p-6 border-t border-[#cccccc4d]">
          <div className="flex items-center gap-4 p-4 rounded-xl bg-muted/50 backdrop-blur-sm">
            <div className="w-10 h-10 bg-sidebar-primary rounded-xl flex items-center justify-center shadow-sm">
