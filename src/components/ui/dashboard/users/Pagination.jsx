...
export default function Pagination({
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 px-4 sm:px-6 py-4 bg-card border-t border-border rounded-b-xl">
      {/* Page Info - Hidden on mobile, compact on tablet */}
      ...
      <div className="flex items-center justify-center gap-2">
        ...
        <button
          onClick={handlePrevious}
          disabled={currentPage === 1 || isLoading}
          className={`group flex items-center gap-2 px-3 sm:px-4 py-2.5 text-sm font-medium rounded-lg border transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed ${
            currentPage === 1 || isLoading
              ? "text-muted-foreground bg-muted border-border"
              : "text-foreground bg-background border-border hover:bg-accent hover:text-accent-foreground shadow-sm hover:shadow-md"
          }`}
          aria-label={t("pagination.previous")}
        >
          {isRTL ? (
        ...
        <div className="flex items-center gap-1 mx-2 sm:mx-3">
          {pageNumbers.map((page, index) => {
            if (typeof page === "string" && page.includes("ellipsis")) {
              return (
                <div
                  key={`ellipsis-${index}`}
                  className="flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 text-muted-foreground"
                  aria-hidden="true"
                >
                  <MoreHorizontal className="h-4 w-4" />
                </div>
            ...
            return (
              <button
                ...
                onClick={() => handlePageClick(page)}
                disabled={isLoading}
                className={`relative w-8 h-8 sm:w-10 sm:h-10 text-sm font-semibold rounded-lg transition-all duration-300 disabled:cursor-not-allowed transform hover:scale-105 active:scale-95 ${
                  isCurrentPage
                    ? "bg-primary text-primary-foreground shadow-lg ring-2 ring-primary/20"
                    : "text-foreground bg-background border border-border hover:bg-accent hover:text-accent-foreground shadow-sm hover:shadow-md"
                }`}
                aria-label={`${t("pagination.goToPage")} ${page}`}
                aria-current={isCurrentPage ? "page" : undefined}
              >
      ...
      {/* Mobile Page Info - Only shown on mobile */}
